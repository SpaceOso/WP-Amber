...
if (have_posts()) {
	...
	while (have_posts()) {
		...
		// $output .= "<a class='' href='" . get_permalink() . "' title='" . the_title_attribute('echo=0') . "'>" . "\n";
		...
		if ( trim($post_thumbnail_html) != '' )
			...
			// $output .= '<span class=""><img src="' . esc_url($thumbnails_default) . '"/></span>';

		// $output .= '<span class="yarpp-thumbnail-date">' . get_the_date() . '</span>';
		$output .= '<span class="related-posts">' . get_the_date() . '</span>';
		$output .= '<span class="related-posts">' . get_the_title() . '</span>';
